...
class EmailsControllerTest < ActionController::TestCase
  ...
  test "response a basic structure on error" do
    post :create, format: :html
    assert_equal({"error"=>"send the correct structure", "to"=>"jhon@example.com[,bill@example.com | bernardo galindo <bernardo.galindo@example.com>]","subject"=>"test", "body"=>"test"}, JSON.parse(response.body))
  end
