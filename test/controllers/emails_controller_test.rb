...
class EmailsControllerTest < ActionController::TestCase
  ...
  test "response a basic structure on error" do
    post :create, format: :html
    assert_equal "{to: jhon@example.com[,bill@example.com | bernardo galindo <bernardo.galindo@example.com>], subject: test, body: test}", response.body
  end
  ...
  test "send email return error to multiples recipients split by space recipients structure" do
    post :create, to: "bernardo466@gmail.com test@example.com", subject: "Hello world", body: "Hi Matte! Sending you an email via this API I just made", format: :json
    assert_equal "please send this structure {to: jhon@example.com[,bill@example.com | bernardo galindo <bernardo.galindo@example.com>], subject: test, body: test}", response.body
  end
